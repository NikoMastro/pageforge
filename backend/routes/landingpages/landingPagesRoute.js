...
class LandingPagesRoute extends BaseRoute {
  constructor() {
    super("lps", requiredMetadataFields);
    this.setupRoutes();
  }
  ...
  async validateDeployment(name) {
    try {
      // Check if the landing page exists in storage directly
      ...
      if (status !== 200) {
        ...
        return {
          error: "An error occurred while validating the landing page",
          status: 500,
          data: null,
        };
      }
  ...
  setupDeployRoute() {
    this.router.post("/deploy/:name", async (req, res) => {
      ...
      // the static landing page. The original implementation is kept below.
...
module.exports = LandingPagesRoute;
