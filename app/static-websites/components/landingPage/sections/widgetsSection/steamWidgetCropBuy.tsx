...
interface SteamWidgetCropBuyProps {
  gameId: string;
  scale?: number;
...
const SteamWidgetCropBuy: React.FC<SteamWidgetCropBuyProps> = ({
  gameId,
  scale = 1,
  language = null,
  className = '',
  ...
  const getCropDimensions = (lang: string) => {
    const buttonWidth = getButtonWidth(lang, 'buy');

    return {
      // Crop position (12px from right edge, 5px from bottom edge)
      cropX: 646 - 12 - buttonWidth, // X position to start crop
      cropY: 190 - 5 - 32,          // Y position to start crop (5px from bottom, height 32px)
      // Crop dimensions
    ...
    >
      ...
      >
        <iframe
          ...
          title={`Steam Widget Buy Button - ${currentLanguage}`}
          loading="lazy"
        />
      </div>
    </div>
...
export default SteamWidgetCropBuy;
