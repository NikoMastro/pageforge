...
interface SteamWidgetCropWishlistProps {
  gameId: string;
  scale?: number;
...
const SteamWidgetCropWishlist: React.FC<SteamWidgetCropWishlistProps> = ({
  gameId,
  scale = 1,
  language = null,
  className = '',
  ...
  const getCropDimensions = (lang: string) => {
    const buttonWidth = getButtonWidth(lang, 'wishlist');

    return {
      // Crop position (16px from right edge, 9px from bottom)
      cropX: 646 - 16 - buttonWidth, // X start crop
      cropY: 190 - 9 - 32,          // Y start crop (9px from bottom, height 32px - 2px removed from top)
      // Crop dimensions
    ...
    >
      ...
      >
        <iframe
          ...
          title={`Steam Widget Wishlist Button - ${currentLanguage}`}
          loading="lazy"
        />
      </div>
    </div>
...
export default SteamWidgetCropWishlist;
