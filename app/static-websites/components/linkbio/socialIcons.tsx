...
const SocialIcon: React.FC<{ platform: string; href: string }> = ({ platform, href }) => {
  const iconClass = "w-8 h-8 text-slate-300 hover:text-white transition-colors";
...
export const SocialIcons: React.FC<SocialIconsProps> = ({ socialLinks }) => {
  ...
  type Entry = { platform: string; url: string } | { platform: 'custom'; url: string; logoUrl?: string };

  const builtInEntries: Entry[] = Object.entries(socialLinks)
    .filter(([key]) => key !== 'custom' && key !== 'order')
    .filter(([, url]) => Boolean(url))
    .map(([platform, url]) => ({ platform, url: url as string }));

  const customEntries: Entry[] = (socialLinks.custom || [])
    .filter(e => e && (e.url || e.logoUrl))
    .map((e, idx) => ({ platform: `custom-${idx}`, url: e.url, logoUrl: e.logoUrl }));

  // Create a map for easy lookup
  const entryMap = new Map<string, Entry>();
  builtInEntries.forEach(entry => entryMap.set(entry.platform, entry));
  customEntries.forEach(entry => entryMap.set(entry.platform, entry));

  // Apply ordering if specified
  let availableLinks: Entry[];
  if (socialLinks.order && socialLinks.order.length > 0) {
    availableLinks = [];
    // Add entries in the specified order
    socialLinks.order.forEach(platform => {
      if (platform === 'custom') {
        // Add all custom entries when 'custom' is in order
        customEntries.forEach(entry => {
          if (entryMap.has(entry.platform)) {
            availableLinks.push(entry);
          }
        });
      } else if (entryMap.has(platform)) {
        availableLinks.push(entryMap.get(platform)!);
      }
    });
    // Add any remaining entries not in the order (fallback)
    entryMap.forEach((entry, platform) => {
      if (!availableLinks.find(e => e.platform === platform)) {
        availableLinks.push(entry);
      }
    });
  } else {
    // Default order: built-in entries first, then custom
    availableLinks = [...builtInEntries, ...customEntries];
  }
  ...
  return (
    <div className="flex justify-center gap-4">
      {availableLinks.map((entry) => (
        (entry as any).platform.startsWith('custom') ? (
          <a
            key={(entry as any).platform}
          ...
          <SocialIcon key={(entry as any).platform} platform={(entry as any).platform} href={(entry as any).url} />
        )
      ))}
    </div>
  );
